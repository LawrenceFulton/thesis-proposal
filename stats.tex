...
\newcommand{\supervisornote}[1]{%
  \fcolorbox{red}{white}{\textcolor{red}{#1}}%
}
...
For each prompt versions there will be two different prompts. The first one (Discussion Prompt) will be used to continue the discussion. The second one (Answer Generation) will be used to generate the numerical answer to the question found in question\_prompt. Thus the question\_prompt placeholder will always only be found in the Answer Generation prompt. The answer from the Discussion Prompt will then be used in future discussions steps and the answer from the Answer Generation will be used later to analyse the change of opinion over time.
...
The third prompt version we change the tone to be very polite. \citeA{yin2024should} found that the politeness of prompts can significantly affect LLM performance. Additionally we will modify the structure of the background\_story to use the the word "You" over "I", thus resulting in sentences such as "Du bist 75 Jahre alt und männlich. Du hast einen Hauptschulabschluss\dots"
